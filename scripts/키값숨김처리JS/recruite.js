 ...
 *  API Key 삭제 → 서버에서 getRecruit.php 로 호출
 * =============================== */

const API_PROXY = "/api/ninehire_api.php"; 
const PER_PAGE = 10;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchJobs(query = "", page = 1) {
    ...
    try {
      const res = await fetch(`${API_PROXY}?page=${page}${query}`, {
        method: "GET",
        headers: { Accept: "application/json" },
      });

      if (!res.ok) throw new Error("서버 호출 오류");
      ...
      if (pageType === "corporate") {
        results = results.filter(
          (job) =>
            job.tags?.includes("대기업") ||
            job.affiliation?.includes("대기업")
        );
      }
