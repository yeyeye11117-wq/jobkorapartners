...
// 지역 페이지 필터 (태그 기반 지역 매칭)
if (pageType === "region") {
  const regionRaw = new URLSearchParams(window.location.search).get("region") || "";
  if (regionRaw) {
    const region = regionRaw.trim(); // "서울", "경기" 등

    results = results.filter((job) => {
      // ① 태그가 문자열 배열인 경우
      if (Array.isArray(job.tags) && typeof job.tags[0] === "string") {
        return job.tags.includes(region);
      }

      // ② 태그가 객체 배열(tags[].name)인 경우
      if (Array.isArray(job.tags)) {
        return job.tags.some(t => (t.name || "").trim() === region);
      }
